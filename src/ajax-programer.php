<?php
    ...
    if(isset($_POST['contact-message'])) {
        $message = substr(htmlspecialchars(trim($_POST['contact-message'])), 0, 100);
        $mess .= '<b>Ваше сообщение</b>' . $message . '<br>';
    }
    $mess .= '<hr>';
    ...
    $mail = new PHPMailer();
    $mail->AddAddress('sale.manager@adagency.com','');   // кому - адрес, Имя
